...
(function (global) {
  ...
  // 统一通知封装 - 使用新的通知管理器
  function notifySuccess(title, text, timer=1200) { 
    if (window.notificationManager) {
      return window.notificationManager.alert('success', title, text, timer);
    }
    return alertAndHide({ icon: "success", title, text, timer }); 
  }
  function notifyError(title, text, timer=3000) { 
    if (window.notificationManager) {
      return window.notificationManager.alert('error', title, text, timer);
    }
    return alertAndHide({ icon: "error", title, text, timer }); 
  }
  function notifyInfo(title, text, timer=1800) { 
    if (window.notificationManager) {
      return window.notificationManager.alert('info', title, text, timer);
    }
    return alertAndHide({ icon: "info", title, text, timer }); 
  }
  function notifyWarning(title, text, timer=2000) { 
    if (window.notificationManager) {
      return window.notificationManager.alert('warning', title, text, timer);
    }
    return alertAndHide({ icon: "warning", title, text, timer }); 
  }
  ...
  function logDebug(){ 
    ...
    } else if (window.logDebug && typeof window.logDebug === 'function') {
      window.logDebug.apply(window, arguments);
    } else {
      try { if (window && window.__IOSAPP_DEBUG__) console.debug.apply(console, arguments); } catch(_){}
    }
  }
  function logWarn(){ 
    if (window.logger && typeof window.logger.warn === 'function') {
      window.logger.warn.apply(window.logger, arguments);
    } else if (window.logWarn && typeof window.logWarn === 'function') {
      window.logWarn.apply(window, arguments);
    } else {
      try { if (window && window.__IOSAPP_DEBUG__) console.warn.apply(console, arguments); } catch(_){}
    }
  }
  function getUDID() {
  ...
  async function stopScreenStream() {
    ...
    try {
      ...
      if (dl) {
        $("#screenshot-preview").html(
          `<div class="d-flex flex-column h-100 justify-content-center align-items-center" style="padding: 20px;">${dl}</div>`
        );
        // 隐藏空状态提示
        $("#screenshot-empty-state").hide();
      } else {
  ...
  async function listCrashes() {
    ...
    try {
      ...
      if (total > items.length) {
        $menu.append(`<div class="small text-muted mb-2 px-2 border-bottom pb-1">仅显示前 ${items.length} 条（共 ${total} 条），请使用搜索精确查找</div>`);
      }
